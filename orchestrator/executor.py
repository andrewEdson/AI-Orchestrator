...
class Executor:
    ...
    def _execute_task_with_retry(self, task: dict[str, Any]) -> AgentResult:
        ...
        # Resolve full code output from upstream dependencies (falls back to summaries)
        dep_ids = task.get("dependencies", [])
        dependency_context = self._get_dependency_code(dep_ids) if dep_ids else None
    ...
    # ------------------------------------------------------------------
    # Dependency context — prefer actual generated code over summaries
    # ------------------------------------------------------------------

    def _get_dependency_code(self, dep_ids: list[str]) -> dict[str, str]:
        """
        Return the actual generated code from each dependency's output file.
        Falls back to the stored summary for any dep whose file is missing.
        Content is capped at 3 000 words per dependency to keep prompts sane.
        """
        result: dict[str, str] = {}
        summaries = self.state.get_dependency_summaries(dep_ids)

        for dep_id in dep_ids:
            try:
                task_state = self.state.get_task(dep_id)
                ...
                if output_file and Path(output_file).exists():
                    ...
                    result[dep_id] = content
                    continue
            except Exception:  # noqa: BLE001
                pass
            # Fallback: use summary if file unavailable
            if dep_id in summaries:
                result[dep_id] = summaries[dep_id]

        return result or None  # type: ignore[return-value]

    # ------------------------------------------------------------------
    # Integration review — cross-task consistency pass
    # ------------------------------------------------------------------

    def _integration_review(self) -> None:
        """
        After all waves complete, run a single Claude pass that reads every
        generated output file and patches any consistency issues: mismatched
        imports, broken cross-module references, or divergent naming.

        Skipped if fewer than 2 tasks completed (nothing to cross-check) or
        if the total content exceeds ~5 000 words (too large to review safely).
        """
        ...
        for task_state in completed:
            ...
            if total_words > 5000:
                log.info(
                    "Integration review skipped — total content exceeds 5 000 words "
                    "(%d tasks collected before limit)", len(sections),
                )
                return
            sections.append(f"### Task: {task_state['id']}\n{content.strip()}")

        if not sections:
            return

        review_prompt = (
            "You are reviewing code generated by multiple independent agents. "
            "Check the files below for cross-module consistency issues only: "
            "missing or incorrect imports, broken function/class references, "
            "inconsistent naming between modules, or interface mismatches. "
            "Output ONLY files that need corrections as fenced code blocks "
            "(e.g. ```python:src/module.py). "
            "If no corrections are needed, output exactly:\n"
            "## Summary\n- All files are consistent, no changes needed.\n\n"
            "---\n\n" + "\n\n".join(sections)
        )
        ...
        log.info("Running integration review across %d completed tasks…", len(completed))
        claude_agent = self.router.get_escalation_agent()
        review_output_file = str(self.output_dir / "integration_review.txt")
        try:
            result = claude_agent.execute(review_task, output_file=review_output_file)
            if result.success:
                written = materialize(result.output, self.target_dir)
                if written:
                    log.info(
                        "Integration review patched %d file(s): %s",
                        len(written),
                        [str(p.relative_to(self.target_dir)) for p in written],
                    )
                else:
                    log.info("Integration review: no patches needed")
        except Exception as exc:  # noqa: BLE001
            log.warning("Integration review failed (non-fatal): %s", exc)
